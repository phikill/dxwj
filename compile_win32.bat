...
SET DIR_JAVA=C:\Program Files (x86)\java\jdk1.5.0_07
SET DIR_DX=C:\Program Files (x86)\Microsoft DirectX SDK (June 2006)

mkdir bin\x86 1>NUL 2>NUL
...
cl /nologo /LD /W4 /EHsc /Febin/x86/WinMan.dll ^
  "/I%DIR_JAVA%\include" "/I%DIR_JAVA%\include\win32" ^
  "src/C++/WinMan.cpp" user32.lib psapi.lib /link /MACHINE:X86

REM cl /nologo /LD /W4 /Febin/x86/Renderer.dll "src/C++/Renderer.cpp" ^
REM  /link /MACHINE:X86
...
link /nologo /DLL DXRenderer.obj Environment3D.obj Primitive.obj ^
 ...
 "%DIR_DX%\lib\x86\d3d9.lib" "%DIR_DX%\lib\x86\d3dx9.lib" ^
 "%DIR_DX%\lib\x86\dxerr9.lib" ^
 /MACHINE:X86 /out:bin/x86/DXRenderer.dll
